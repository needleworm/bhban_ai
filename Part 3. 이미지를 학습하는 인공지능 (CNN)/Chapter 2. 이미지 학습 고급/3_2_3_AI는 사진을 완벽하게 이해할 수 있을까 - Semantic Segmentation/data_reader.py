...
def save_segmentation_results(image, mask, graph):
    prediction = graph.predict(image)[:, :, :, 0]
    pred_mask = (prediction > 0.5).astype(np.uint8) * 255
    image = (image * 255).astype(np.uint8)
    mask = (mask * 255).astype(np.uint8)

    mask_template = np.zeros_like(image)
    mask_template[:, :, :, 0] = mask
    mask_template[:, :, :, 1] = mask
    mask_template[:, :, :, 2] = mask
    mask = mask_template

    pred_mask_template = np.zeros_like(image)
    pred_mask_template[:, :, :, 0] = pred_mask
    pred_mask_template[:, :, :, 1] = pred_mask
    pred_mask_template[:, :, :, 2] = pred_mask
    pred_mask = pred_mask_template
